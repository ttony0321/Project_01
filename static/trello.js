    ...
    $(document).on("submit", ".add-card-form", function (e) {
        ...
        if (content.val().length > 0) {
            ...
            $(this).parent().children('.card-wrap').append(`<div class="list-box-card">
                                                <a class="list-card">
                                                <span class="material-icons edit-icon" onclick="uuid()">
                                                    clear
                                                </span>
        ...
        success: function (response) {
            if (response['result'] == 'success') {
                for (let i = 0; i < response['card_show'].length; i++) {
                    ...
                    console.log(response['card_show'][i]['card_id'], response['card_show'][i]['card_id'], response['card_show'][i]['card_uuid'])
                    edit(response['card_show'][i]['card_id'], response['card_show'][i]['card_uuid'])
...
function edit(card_id, card_uuid) {
    // let edit_title = $(this).children('.card-editor-box').children().children(".card-edit-textarea").val()
    let edit_title = $(".card-edit-textarea").val()
    console.log(edit_title)
    $.ajax({
        type: "PUT",
        url: "/card",
